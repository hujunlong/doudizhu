...
func (client *Client) ZMyGet(key string, start string, end string, number string) ([][]byte, error) {
	fmt.Println(key, start, end)
	res, err := client.sendCommand("ZREVRANGEBYSCORE", key, end, start, "LIMIT", "0", number)
	if err != nil {
		return nil, err
	}
	return res.([][]byte), nil
